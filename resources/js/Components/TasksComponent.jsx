...
const LocalTaskForceTaskView = () => {
  ...
  const openModal = async (task) => {
    ...
    if (task.status !== "in-progress" && task.status !== "completed") {
      try {
        ...
        setTasks((tasks) => tasks.map((t) => (t.id === task.id ? { ...t, status: "in-progress" } : t)))
  ...
  const handleRatingChange = async (taskId, rating) => {
    try {
      ...
      await logActivity("update", `Task self-rated as ${rating}/5`, "Task", taskId);
  ...
  return (
    <div className="container mx-auto p-6 max-w-5xl">
      <h1 className="text-4xl font-bold mb-10 text-gray-800 text-center">Assigned Tasks</h1>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {tasks.map((task) => {
            ...
            if (task.status == "in-progress" || task.status == "pending") {
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    <h2 className="text-2xl font-semibold text-gray-700 pr-4">{task.title}</h2>
                    <span className="text-2xl">{getStatusIcon(task.status)}</span>
                  </div>
                  <p className="text-gray-600 mb-6 text-2xl">{task.description}</p>
                  {indicator && (
                    <>
                      <p className="text-lg text-gray-500">Indicator: {indicator.description}</p>
                      {indicator.documents ? (
                        <div className="mt-2">
                          <p className="text-sm font-medium text-green-500">Has Linked Document.</p>
                        </div>
                      ) : (
                        <p className="text-sm pt-2  font-medium text-red-500">No document.</p>
                      )}
                    </>
                  )}
                  {taskRatings[task.id] && (
                    <div className="mt-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm text-gray-600">Self-rated: {taskRatings[task.id]}/5</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2.5">
                        <div
                          className="bg-blue-600 h-2.5 rounded-full"
                          style={{ width: `${(taskRatings[task.id] / 5) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  )}
              ...
              return <p className="text-center mt-8 text-gray-600 text-xl">No tasks assigned to you at the moment.</p>
            }
          })}
        </div>
      )}
      <div>
        <br />
        <br />
        <h1 className="text-4xl font-bold mb-10 text-gray-800 text-center">Completed Tasks</h1>
        <div className="grid gap-8 md:grid-cols-2">
          {tasks.map((task) => {
            console.log(task)
            if (task.status == "completed") {
              ...
              return <p className="text-center mt-8 text-gray-600 text-xl">No completed task yet.</p>
            }
          })}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold text-gray-700">{selectedTask.title}</h2>
            </div>

            <p className="text-gray-600 mb-6 text-xl">{selectedTask.description || "No description provided."}</p>
            <p className="text-lg text-gray-500 mb-6">Task: {selectedTask.task || "No task details available."}</p>
            <div className="mb-6">
              <h3 className="text-lg font-medium text-gray-700 mb-2">Indicator Document</h3>
...
>
    ...
    <span className="group-hover:underline">View Document</span>
        ...
        <div className="mt-2">
            <p className="text-sm font-medium text-green-500">Document has been uploaded</p>
        </div>
    </div>
) : (
    <div className="bg-gray-100 p-4 rounded-md">
        <p className="text-gray-500">No document available for this indicator.</p>
        <FileUploadDialog onUpload={handleFileUpload} buttonText="Upload Document" />
    </div>
)}
              {isUploading && (
            ...
            <div className="mb-6">
              <h3 className="text-lg font-medium text-gray-700 mb-2">Self-Assessment Rating</h3>
              <div className="flex flex-col space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">Low</span>
                  <span className="text-sm font-medium text-gray-700">
                    {taskRatings[selectedTask.id] ? `${taskRatings[selectedTask.id]}/5` : "Not rated yet"}
                  </span>
                ...
                <input
                  ...
                  onChange={(e) => handleRatingChange(selectedTask.id, Number.parseInt(e.target.value))}
              ...
              <p className="text-sm text-gray-500 mt-2">Rate how well you think you completed this task</p>
            </div>
