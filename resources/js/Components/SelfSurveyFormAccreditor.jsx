...
const SelfSurveyFormAccreditor = () => {
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      if (response.data && response.data.program_id) {
        await fetchAreas(response.data.program_id)
      } else {
        setError("Your account is not associated with any program. Please contact your administrator.")
        setLoading(false)
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (currentUser?.program_id) {
      fetchAreas(currentUser.program_id)
    }
  }, [currentUser])
...
export default SelfSurveyFormAccreditor
